  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:ecr:${var.aws_region}:${data.aws_caller_identity.current.account_id}:repository/${var.project}-app"
      },
      {
        Sid      = "EKSDescribe"
        Effect   = "Allow"
      ...
      },
      {
        ...
        Resource = [
          "arn:aws:s3:::autoforge-tfstate-${data.aws_caller_identity.current.account_id}",
          "arn:aws:s3:::autoforge-tfstate-${data.aws_caller_identity.current.account_id}/*"
        ]
      },
      {
        ...
        Resource = "arn:aws:dynamodb:${var.aws_region}:${data.aws_caller_identity.current.account_id}:table/autoforge-tf-locks"
      }
    ]
  })
}
