...
# key. Terraform generates the DB password itself (random_password) so it
# never passes through your shell history or a .tfvars file.
# =============================================================================
...
resource "random_password" "db_password" {
  length  = 24
  special = true
  # RDS disallows these chars in the master password
  override_special = "!#$%^&*()-_=+[]{}<>:?"
}
...
resource "aws_secretsmanager_secret" "app_secrets" {
  ...
  recovery_window_in_days = 0 # allow immediate re-create during dev teardown cycles
...
# it -- and only it -- can read this one secret. Scoped with a resource ARN
# condition, not "secretsmanager:*" on "*".
