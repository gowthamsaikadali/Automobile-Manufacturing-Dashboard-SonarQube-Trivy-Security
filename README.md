...
Unzip this bundle, then from inside the unzipped `autoforge` folder:
...
```powershell
git add .
git commit -m "Initial commit: AutoForge app + Terraform + Helm + CI/CD security hardening"
...
Everything from here on assumes that repo now exists and is your `origin`.

---

## Step 0. Bootstrap Terraform remote state (one-time, before anything else)

Since you're starting from scratch, the S3 bucket + DynamoDB table that
`terraform/infra/main.tf` expects as its backend don't exist yet. Create
them first, with local state (no backend), just this once:
...
This creates `autoforge-tfstate-762131619075` (versioned, encrypted,
public access blocked) and the `autoforge-tf-locks` DynamoDB table. Now
`terraform/infra`'s `backend "s3"` block will work.
...
## Step 3. Terraform — provision RDS, IAM, Secrets Manager, ACM, WAF
...
## Step 5. Wire up the Helm values
...
## Step 6. GitHub Actions secrets (repo settings → Secrets and variables)
